...
export class ShahkarService {
  ...
  constructor(configService: ConfigService) {
    this.baseUrl = (configService.get<string>('SHAHKAR_BASE_URL') || configService.get<string>('config.SHAHKAR_BASE_URL'))?.trim() || '';
    this.apiKey = (configService.get<string>('SHAHKAR_API_KEY') || configService.get<string>('config.SHAHKAR_API_KEY'))?.trim() || '';
  }

  getCapability(): { available: boolean; reason?: 'configuration_missing' } {
    return this.baseUrl && this.apiKey ? { available: true } : { available: false, reason: 'configuration_missing' };
  }
